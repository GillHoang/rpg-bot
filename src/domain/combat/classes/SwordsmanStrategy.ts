...
export class SwordsmanStrategy extends NullClassStrategy {
	...
	override onHitLanded(ctx: StrategyContext, resolved: ResolvedHit): void {
		...
		if (current < ATK_STACK_MAX) {
			...
			ctx.log(`⚔️ Swordsman Passive: ATK increased by ${Math.round((next - current) * 100)}%.`);
		}
		...
		ctx.log(`⚔️ Swordsman Passive — applied Bleed. Stack: ${stacks}/${BLEED_MAX_STACKS} (${pct}% ATK/turn).`);
	}
}
