...
/** Voluntary supporter contributions. Payment amounts unlock cosmetic/social thank-you benefits only. */
export const donationOrders = pgTable(
	'donation_orders',
	{
		orderId: text('order_id').primaryKey(),
		/** Keep donation history even when an RPG account is reset or absent. */
		discordId: text('discord_id').notNull(),
		amount: integer('amount').notNull(),
		paymentCode: text('payment_code').notNull(),
		/** Null means the contribution is valid but below the first configured tier. */
		tierId: text('tier_id'),
		/** Discord interaction ID used to make command retries idempotent. */
		requestId: text('request_id'),
		status: text('status').notNull().default('pending'),
		sepayTransactionId: text('sepay_transaction_id'),
		paidAt: timestamp('paid_at', { mode: 'date', withTimezone: false }),
		createdAt: timestamp('created_at', { mode: 'date', withTimezone: false })
			.notNull()
			.default(sql`now()`),
		updatedAt: timestamp('updated_at', { mode: 'date', withTimezone: false })
	...
	(table) => [
		unique('donation_orders_payment_code_unique').on(table.paymentCode),
		unique('donation_orders_request_id_unique').on(table.requestId),
		unique('donation_orders_sepay_transaction_unique').on(table.sepayTransactionId),
	],
);

/** Durable idempotency record for every accepted provider webhook. */
export const donationWebhookReceipts = pgTable(
	'donation_webhook_receipts',
	{
		receiptId: text('receipt_id').primaryKey(),
		providerEventId: text('provider_event_id').notNull(),
		transactionId: text('transaction_id').notNull(),
		orderId: text('order_id').references(() => donationOrders.orderId, { onDelete: 'set null' }),
		status: text('status').notNull().default('received'),
		payload: jsonb('payload').notNull(),
		receivedAt: timestamp('received_at', { mode: 'date', withTimezone: false })
			.notNull()
			.default(sql`now()`),
		processedAt: timestamp('processed_at', { mode: 'date', withTimezone: false }),
	},
	(table) => [
		unique('donation_webhook_provider_event_unique').on(table.providerEventId),
		unique('donation_webhook_transaction_unique').on(table.transactionId),
	],
);

/** Outbox job used to provision Keygate supporter entitlements exactly once. */
export const donationProvisioningJobs = pgTable(
	'donation_provisioning_jobs',
	{
		jobId: text('job_id').primaryKey(),
		orderId: text('order_id')
			.notNull()
			.references(() => donationOrders.orderId, { onDelete: 'cascade' }),
		status: text('status').notNull().default('pending'),
		attempts: integer('attempts').notNull().default(0),
		nextAttemptAt: timestamp('next_attempt_at', { mode: 'date', withTimezone: false })
			.notNull()
			.default(sql`now()`),
		lockedAt: timestamp('locked_at', { mode: 'date', withTimezone: false }),
	...
	(table) => [unique('donation_provisioning_order_unique').on(table.orderId)],
);

/** Verified Keygate result written before an order becomes active. */
export const donationGrantReceipts = pgTable(
	'donation_grant_receipts',
	{
		operationId: text('operation_id').primaryKey(),
		orderId: text('order_id')
			.notNull()
			.references(() => donationOrders.orderId),
		grantId: text('grant_id'),
		subject: text('subject').notNull(),
		planSlug: text('plan_slug').notNull(),
		expiresAt: timestamp('expires_at', { mode: 'date', withTimezone: false }),
		entitlements: jsonb('entitlements').notNull(),
		verifiedAt: timestamp('verified_at', { mode: 'date', withTimezone: false })
			.notNull()
			.default(sql`now()`),
	},
	(table) => [unique('donation_grant_receipts_order_unique').on(table.orderId)],
);
