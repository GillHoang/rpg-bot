...
export const HELP_PAGES: readonly HelpPage[] = [
	...
	},
	{
		...
		body: [
			...
			`**${ICONS.deity.ascension} \`/deity ascend user_deity_id:<id>\`** — khi đủ 10 Sigil: nộp Essence + Credux để **Ascension** (prestige).`,
	...
	},
	{
		...
		body: [
			...
			`**${ICONS.duel.challenge} \`/duel opponent:@user [stake]\`** — thách đấu 1v1 trực tiếp; đối thủ bấm **Chấp nhận** trong 60 giây. Có cược (tối thiểu 1.000 Credux): cả hai phải đủ tiền, người thắng ăn cả pot.`,
			`**${ICONS.ranked.profileBadge} \`/ranked\`** — PvP bất đồng bộ: \`fight\` ghép bạn với loadout của một người chơi ngẫu nhiên cùng tầm rating (Elo). Bracket: Mortal → Champion (1100) → Demigod (1400) → Ascendant (1700) → Divine (2000). \`stats\` xem rating hiện tại · \`claim\` nhận thưởng tuần = **Valor Medals** + rương.`,
			`**${ICONS.menu.shop} \`/pvp\`** — \`shop\` xem hàng mua bằng Valor Medals: Change-Class Token (120) · Diamond Chest (60) · khung/banner/vòng summon/title giới hạn (40–150, tối đa 1 mỗi season). \`buy item:<tên>\` để mua.`,
			`**${ICONS.menu.casino} \`/casino <game> bet:1–500.000\`** — coin_toss · dice_roll · slot_machine · baccarat (chọn phe) chơi nhanh 1 ván; **blackjack** và **crash** chơi bằng nút bấm (hết hạn 60 giây) — crash thì cash out kịp trước khi nổ, không thì mất trắng.`,
		].join('\n\n'),
	},
	{
		title: 'Cosmetic & khác',
		body: [
			...
			`**${ICONS.combatClass.change} \`/class\`** — \`info class:<tên>\` xem trước nội tại/chỉ số · \`change new_class:<tên>\` đổi class tốn **Change-Class Token** (mua ở \`/pvp shop\`).`,
			`**${ICONS.gear.list} \`/cosmetic\`** — \`list\` xem catalog · \`equip id:<#>\` trang bị khung ảnh, banner, vòng summon… Khoá theo **believer level**: tier Chosen cần lv 5, Eternal cần lv 10.`,
			`**${ICONS.gear.titles} \`/title\`** — \`list\` xem title · \`equip id:<#>\` đeo (\`id 0\` = tháo). Title kiếm từ ranked và shop PvP.`,
			`**${ICONS.deity.believer} Believer level** — EXP tăng dần từ \`/daily\`, thắng raid/duel/ranked và làm quest (cap 500 EXP/ngày) — level càng cao mở càng nhiều cosmetic tier.`,
			`**${ICONS.help.ping} \`/ping\`** — đo độ trễ WebSocket / REST / PostgreSQL.`,
