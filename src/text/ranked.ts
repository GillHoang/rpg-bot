...
// fight
export const RANKED_NOT_REGISTERED = 'Dùng /register trước.';
export const RANKED_NO_CHARACTER = 'Dùng /create trước.';
...
export const RANKED_CLAIM_OK = (bracket: string, creux: string, valor: number): string =>
	`🏅 Weekly reward (${bracket}): +${creux} Credux · +${valor} Valor Medals`;
...
export const RANKED_STATS_HEADER = (rating: number, bracket: string, peak: number): string =>
	`🏅 **Ranked** — Rating **${rating}** (${bracket}) · Peak **${peak}**`;
